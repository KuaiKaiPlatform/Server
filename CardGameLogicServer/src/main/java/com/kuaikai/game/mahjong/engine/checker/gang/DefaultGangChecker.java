...
public class DefaultGangChecker implements GangChecker {
	...
	public List<BaseOperation> checkBuGangOperations(BaseOperation oper) {
		...
		for(CardGroup group : cardContainer.getCardGroups()) {
			...
			if(cardNum == almighty && !room.getCreateRoomParam().getSettingBool(GameSetting.ALMIGHTY_GANG)) continue;	// 不能杠万能牌
	...
	protected boolean preCheckBuGang(BaseOperation oper) {
		// 听后补杠
		if(player.getMjPlayer().isBaoTing() && !room.getCreateRoomParam().getSettingBool(GameSetting.TING_HOU_BU_GANG))  return false;
		
		switch(oper.getOperType()) {
		case OperType.MO :	// 摸后补杠
			return player.equals(oper.getPlayer());
		case OperType.CHI :		// 吃后补杠
			return player.equals(oper.getPlayer()) && room.getCreateRoomParam().getSettingBool(GameSetting.CHI_HOU_BU_GANG);
		case OperType.PENG :	// 碰后补杠
			return player.equals(oper.getPlayer()) && room.getCreateRoomParam().getSettingBool(GameSetting.PENG_HOU_BU_GANG);
		}
	...
	protected boolean preCheckAnGang(BaseOperation oper) {
		// 听后暗杠
		if(player.getMjPlayer().isBaoTing() && !room.getCreateRoomParam().getSettingBool(GameSetting.TING_HOU_AN_GANG))  return false;
		
		switch(oper.getOperType()) {
		case OperType.MO :	// 摸后暗杠
			return player.equals(oper.getPlayer());
		case OperType.CHI :		// 吃后暗杠
			return player.equals(oper.getPlayer()) && room.getCreateRoomParam().getSettingBool(GameSetting.CHI_HOU_AN_GANG);
		case OperType.PENG :	// 碰后暗杠
			return player.equals(oper.getPlayer()) && room.getCreateRoomParam().getSettingBool(GameSetting.PENG_HOU_AN_GANG);
		}
	...
	public DianGangOperation checkDianGangOperation(BaseOperation daOper) {
		...
		int almighty = room.getEngine().getAlmightyCardNum();
		if(cardNum == almighty && !room.getCreateRoomParam().getSettingBool(GameSetting.ALMIGHTY_GANG)) return null;	// 不能杠万能牌
	...
	protected boolean preCheckDianGang(BaseOperation oper) {
		// 听后接杠
		if(player.getMjPlayer().isBaoTing() && !room.getCreateRoomParam().getSettingBool(GameSetting.TING_HOU_JIE_GANG))  return false;
