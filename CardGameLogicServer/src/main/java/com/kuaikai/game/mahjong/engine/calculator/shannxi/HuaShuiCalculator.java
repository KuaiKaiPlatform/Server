...
public class HuaShuiCalculator extends Calculator {
	...
	public List<CommonCalculatorDetail> createHuDetails(HuOperation oper) {
		...
		if (room.getCreateRoomParam().getSettingBool(GameSetting.HUANG_ZHUANG_DOUBLE) && room.getEngine().getLastSetResult().isHuangZhuang()) {
		...
		int yu = room.getCreateRoomParam().getSettingInt(GameSetting.HUA_SHUI_YU);
		if (yu > 0) {
