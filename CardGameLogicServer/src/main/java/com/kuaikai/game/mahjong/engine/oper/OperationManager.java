...
public class OperationManager {
	...
	public OperationManager(MahjongDesk room) {
		this.room = room;
	}
	...
	private List<BaseOperation> findCanExecuteOperationsByPriority() {
		...
		// 一炮多响计数
		int huCount = 0;
		if(room.getCreateRoomParam().getSettingBool(GameSetting.MULTIPLE_HU)) {
			for(BaseOperation oper : canExecuteOperations) {
		...
		if(huCount > 1 && multipleHuOperations == null) {
			multipleHuOperations = new MultipleHuOperations(room);
		}
