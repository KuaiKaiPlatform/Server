...
public class GuoZiCalculator extends DiCalculator {
	...
	public int getInitDiFen() {
		return room.getEngine().checkPlayMode(PlayMode.DI)?100:0;
	}
	
	@Override
	public CommonCalculatorDetail createGangDetail(BaseOperation oper) {
		boolean bankerDouble = room.getCreateRoomParam().getSettingBool(GameSetting.BANKER_DOUBLE);
		...
		case OperType.DIAN_GANG:	// 点杠
			...
			if (room.getCreateRoomParam().getSettingBool(GameSetting.DIAN_PAO_PAY_DIAN_GANG)) {
				// 先设置为杠分三家付，胡牌时若点炮者未听牌则包三家
				gangCalculatorDetail.setPayerAll();
			} else {
				// 点杠者已听牌，杠分三家付；未听牌则包三家
				if (oper.getPrePlayer().getMjPlayer().isBaoTing()) {
					gangCalculatorDetail.setPayerAll();
				} else {
	...
	private void dianPaoPayDianGang(MahjongPlayer dianPlayer) {
		if (!room.getCreateRoomParam().getSettingBool(GameSetting.DIAN_PAO_PAY_DIAN_GANG)) return;
