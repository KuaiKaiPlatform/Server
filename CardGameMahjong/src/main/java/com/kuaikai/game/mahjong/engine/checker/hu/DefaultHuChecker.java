...
/*		if(logger.isDebugEnabled())
		...
		if(HuModesChecker.tingAll(player)) {
			if(!desk.getSetting().getBool(CardGameSetting.TING_ALL_DIAN) && !qiangGang) return false;		// 不能点炮胡 
			if(!desk.getSetting().getBool(CardGameSetting.TING_ALL_QIANG_GANG) && qiangGang) return false;	// 不能抢杠胡
		}
		...
		// 普通点炮胡，房间规则为只能自摸
		if (desk.getSetting().getBool(CardGameSetting.ZI_MO_ONLY)) return false;
	...
	public boolean louHuCheck(BaseOperation oper) {
		if(desk.getSetting().getBool(CardGameSetting.NO_LOU_HU)) return true;		// 无漏胡限制
		if(!player.getSetAttrs().contains(MahjongPlayer.SetAttr.LOU_HU_CARDS)) return true;	// 玩家当前不处于漏胡状态
		if(desk.getSetting().getInt(CardGameSetting.LOU_HU_NUM) < 0) return false;	// -1 漏胡所有
