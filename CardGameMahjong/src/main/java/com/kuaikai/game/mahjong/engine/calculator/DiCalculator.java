...
public abstract class DiCalculator extends Calculator {
	...
	protected void calculate() {
		if(!desk.getEngine().checkPlayMode(PlayMode.DI)) {
			super.calculate();
			return;
		}
...
/*		// 计算各项分数，得到所有玩家得失分，记下有得分项的玩家
	...
	protected boolean checkNegative() {
		...
		for(Map.Entry<Integer, Integer> entry : userScores.entrySet()) {
			// 检查当前底的底分扣除输分，是否为负数
			MahjongPlayer player = (MahjongPlayer)desk.getPlayerById(entry.getKey());
			GamePoints diPoints = player.getGameResult().getCurrentDiGamePoints();
	...
	protected void handleGameResultWithSinglePositive() {
		...
		for (PlayerSetResult playerSetResult : playerSetResults.values()) {
			...
			if(setPoint < 0) {	// 输家，底分不够时，归0，重新计算实际输分
				PlayerGameResult playerGameResult = playerSetResult.getPlayer().getGameResult();
	...
	protected void calculateDi() {
		...
		for(GamePlayer p : desk.getAllPlayers()) {
			...
			remainPoints.put(player.getId(), player.getGameResult().getCurrentDiGamePoints().getPoint(INDEX_DI));
