...
public class DefaultChiChecker implements ChiChecker {
	...
	public List<List<MJCard>> checkShunZis(BaseOperation oper) {
		...
		if(target.isAlmighty() && !desk.getSetting().getBool(CardGameSetting.ALMIGHTY_CHI)) return null;	// 不能吃万能牌
		...
		// 万能牌还原可吃时，不转为100
		Map<Integer, Integer> card2count = PaiXinHelper.countCards(player.getCardContainer().getHandCards(),
				desk.getSetting().getBool(CardGameSetting.ALMIGHTY_CHI)?-1:desk.getEngine().getAlmightyCardNum());
	...
	private List<MJCard> createArr(int card1, int card2) {
		...
		list.add(player.getCardContainer().findHandCard(card1));
		list.add(player.getCardContainer().findHandCard(card2));
		return list;
	}
